...
export const showVotingPage = (pollId: number) => {
  ...
  const currentUserVote = () =>
    currentUser.voted.find((votes) => votes.pollId === poll.id) ?? null;
  ...
  const updateVotesCountElement = () => {
    ...
    votesCountElement.textContent = updateVotesCount();
  }
    ...
    poll.pollChoices.forEach((option) => {
      const optionProgressPercentage = (() => {
        const progress = Number(
          ((option.choosedBy.length * 100) / updateVotesCount()).toFixed()
        );
      ...
      const setProgressBarPercentage = () => {
  ...
  const retractUserVote = () => {
    ...
    const userVote: voted | null = currentUserVote();
